...
echo 'set /files/usr/local/airflow/airflow.cfg/core/fernet_key '${FERNET_KEY:=$(python3.6 -c "from cryptography.fernet import Fernet; FERNET_KEY = Fernet.generate_key().decode(); print(FERNET_KEY)")} >> tmp
